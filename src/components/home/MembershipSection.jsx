...
const ICON_COMPONENTS = [Building2, Handshake, Globe, Heart, Globe, Handshake];

export default function MembershipSection({ content }) {
  const sectionTitle = content?.sectionTitle ?? "Membership Benefits";
  const subtitle =
    content?.subtitle ??
    "Joining Clubhouse is not just about gaining access to an exclusive network; it's about becoming part of a dynamic ecosystem designed to elevate your professional and personal life.";
  const benefits = content?.benefits?.length
    ? content.benefits
    : DEFAULT_BENEFITS;
  const ctaLabel = content?.ctaLabel ?? "Explore Membership Options";
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-16 relative z-10">
          {benefits.map((benefit, index) => {
            const Icon = ICON_COMPONENTS[index % ICON_COMPONENTS.length];
            ...
            return (
              ...
              <div key={index} className="h-full">
                ...
                <div className="bg-white rounded-xl overflow-hidden shadow-xl h-full group hover:shadow-2xl transition-shadow duration-500">
                  ...
                  <div
                    className={`h-24 bg-gradient-to-r from-theme to-theme/80 flex items-center justify-center relative overflow-hidden`}
                  >
                    {/* Removed Animated background particles */}

                    {/* Icon */}
                    <div className="w-28 h-28 bg-theme/10 rounded-full flex items-center justify-center text-theme z-10">
                  ...
                  {/* Card content */}
                  <div className="p-6">
                    <h3 className="text-xl font-bold mb-3 text-theme font-display">
                      {benefit.title}
                    </h3>
